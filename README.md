...
## Why does it need so much RAM ?
  ...
  - ### This platform is planned to be implemented in such way that users don't have to worry about the technology behind (until certain threshold).
    ...
    - we will have a lot of categorization data for each card. That data is generated by people that is actually contributing with work, so is not good idea to charge those people for that data's RAM. We need the system to store that for them.
...
## What are the timing for the releases ?
  Behind this project it's just me working hard on every free time I have (weekends, licences, hollydays, after midnight, etc). I'm really pasionated about this project and I will make it work for sure, but for now I can't guarantee timing because it goes slow (but as fast as I can).
